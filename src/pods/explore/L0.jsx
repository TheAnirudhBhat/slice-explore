...
import { useL1 } from '../../components/L1Stack.jsx'; // shared L1 overlay stack — avatar → Profile
    ...
    /* ============= TYPOGRAPHY ============= */
    const T = {
      h2: { fontSize: 24, lineHeight: '32px', fontWeight: 500, letterSpacing: '0.48px', color: 'var(--text-primary)' },
      h3: { fontSize: 20, lineHeight: '24px', fontWeight: 500, letterSpacing: '0.40px', color: 'var(--text-primary)' },
      h4: { fontSize: 16, lineHeight: '20px', fontWeight: 500, letterSpacing: '0.32px', color: 'var(--text-primary)' },
      body: { fontSize: 16, lineHeight: '24px', fontWeight: 400, letterSpacing: '0.32px', color: 'var(--text-primary)' },
      bodySm: { fontSize: 14, lineHeight: '20px', fontWeight: 400, letterSpacing: '0.28px', color: 'var(--text-primary)' },
      caption: { fontSize: 12, lineHeight: '16px', fontWeight: 400, letterSpacing: '0.24px', color: 'var(--text-tertiary)' },
      meta: { fontSize: 10, lineHeight: '12px', fontWeight: 400, letterSpacing: '0.40px', color: 'var(--text-tertiary)', textTransform: 'uppercase' },
      ...
      display: { fontSize: 40, lineHeight: '48px', fontWeight: 500, letterSpacing: '-0.4px', color: 'var(--text-primary)' },
    };
    const CARD_SHADOW = '0px 2px 32px 0px rgba(0,0,0,0.05)';
    const CARD_BORDER = '1px solid var(--outline-subtle)';
    ...
    /* ScreenShell — the STANDALONE explore chrome (restored for the bleed
       exploration). The shared skill shell reserves 54px at the top for its
       status bar, which would stop For-You heroes from reaching the top. So the
       explore pod-root cancels that reserve (negative margin, see ExploreL0) and
       fills from y=0, and this ScreenShell runs the standalone model: an ABSOLUTE
       AppBarL0 at --appbar-top (transparent + white title on dark heroes via
       `darkBg`) over a full-height scroll, so F/N/P/T/L heroes bleed up under the
       status bar exactly as before. EXPLORATION-ONLY — not a slice pattern, so
       none of this lives in the skill proto. */
      ...
      return (
        <div className="absolute inset-0" style={{ background: 'var(--page-bg)' }}>
          ...
          <div className="scrollbar-hide screen-scroll" onScroll={onScroll} style={{
            ...
            paddingBottom: 200,
            overflowY: 'auto',
            // none (not contain): kill the elastic vertical rubber-band on the page
            // itself. `contain` only stops scroll CHAINING to the pager — the page
            // still bounced past its top/bottom. (user: "remove vertical over scroll
            // on the pages", 2026-06-02)
    ...
    const DividerBig = () => {
      ...
      return <div style={{ height: 8, background: 'var(--slate-10)', width: '100%' }} />;
    };
    const DividerInset = () => {
      const { hide } = React.useContext(DividerCtx);
      if (hide) return null;
      return <div style={{ borderTop: '1px solid var(--outline-subtle)', marginLeft: 76 }} />;
    };
    const DividerFull = () => {
      const { hide } = React.useContext(DividerCtx);
      if (hide) return null;
      return <div style={{ borderTop: '1px solid var(--outline-subtle)' }} />;
    };
        ...
        {cta && (
          ...
          <button className="tap" aria-label={cta} style={{
            ...
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden>
              <path d="M9 6l6 6-6 6" stroke="var(--text-secondary)" strokeWidth="2"
    ...
    const TagSubtle = ({ children, intent = 'info' }) => {
      const map = {
        ...
        neutral: { color: 'var(--text-secondary)', bg: '#F0F4F7' },
      };
    ...
    const GlyphMore = ({ color = 'var(--text-secondary)' }) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill={color}>
        <circle cx="6" cy="12" r="2" /><circle cx="12" cy="12" r="2" /><circle cx="18" cy="12" r="2" />
      </svg>
    );
    const GlyphSettings = ({ color = 'var(--text-secondary)' }) => (
    ...
    const GlyphHelp = ({ color = 'var(--text-secondary)' }) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2">
    ...
    const GlyphShield = ({ color = 'var(--text-secondary)' }) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2">
        <path d="M12 3 4 6v6c0 5 3.5 8.5 8 9 4.5-.5 8-4 8-9V6l-8-3z" />
      </svg>
    );
    const GlyphInfo = ({ color = 'var(--text-secondary)' }) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2">
        <circle cx="12" cy="12" r="9" />
    ...
    const GlyphCard = ({ color = 'var(--text-secondary)' }) => (
    ...
    const GlyphMobile = ({ color = 'var(--text-secondary)' }) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    ...
    const GlyphPlus = ({ color = 'var(--text-secondary)' }) => (
    ...
    const ListItemTransaction = ({ asset, bg, glyph, name, sub1, sub2, amount, amountColor, trailing }) => (
      <div className="tap" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 24px' }}>
        ...
        {trailing !== undefined ? trailing : (
          <div style={{ ...T.h4, color: amountColor || 'var(--text-primary)' }}>{amount}</div>
        )}
      </div>
    );
    ...
    const ExploreMedium = ({ subtext, title, icon }) => (
      <button className="tap relative overflow-hidden"
        style={{
          background: 'var(--surface)',
    ...
    const L0CardLarge = ({ label, amount, insightText, ctaTitle, ctaSubtitle, ctaText }) => (
      <div style={{
        background: 'var(--surface)', border: CARD_BORDER, borderRadius: 16, boxShadow: CARD_SHADOW,
        padding: 24, display: 'flex', flexDirection: 'column', gap: 32
      }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          <span style={{ ...T.btnSm, color: 'var(--text-tertiary)' }}>{label}</span>
      ...
      <div style={{
        ...
        {ctaText && (
          <button className="tap" style={{
            ...
            padding: '8px 16px', background: 'var(--slate-30)', border: 'none', borderRadius: 100,
            ...T.btnSm, color: 'var(--text-primary)', cursor: 'pointer',
          }}>{ctaText}</button>
        )}
        {illustrationAsset && (
    ...
    const MarketingCard = ({ title, subtitle, glyph, brand = false }) => (
      <button className="tap" style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ ...T.h4, color: brand ? 'white' : 'var(--text-primary)' }}>{title}</div>
          <div style={{
            ...T.caption, marginTop: 2,
            color: brand ? 'rgba(255,255,255,0.7)' : 'var(--text-tertiary)'
          }}>{subtitle}</div>
        </div>
    ...
    /* Bills shortcut grid */
    /* Bill tiles: 54px chip (BillAvatar) holding a 24px glyph. SAME logic as the
       skill proto's Recharge & bills — inline SVG glyphs (currentColor + baked
       V-500 accent), NOT raster PNGs, imported from the linked kit. The chip
       drives `color`, so the glyph strokes flip dark↔light with the theme (the
       old two-tone bill_v2_*.png had baked-dark strokes that vanished on dark). */
    const BILL_ICONS = [
      { glyph: <CreditCardIcon size={24} />, t: 'Credit\ncard' },
      { glyph: <ElectricityIcon size={24} />, t: 'Electricity\nbill' },
      { glyph: <MobileIcon size={24} />, t: 'Mobile\nrecharge' },
      { glyph: <MoreIcon size={24} />, t: 'View\nmore' },
    ];

    const BillsCompositeCard = () => (
      <div style={{ background: 'var(--surface)', boxShadow: CARD_SHADOW, border: CARD_BORDER, borderRadius: 16, padding: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginTop: 20 }}>
          {BILL_ICONS.map((b, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <div style={{ ...T.caption, textAlign: 'center', marginTop: 8, whiteSpace: 'pre-line', color: 'var(--text-secondary)' }}>
                {b.t}
              </div>
            </div>
          ))}
        </div>
        <div style={{ borderTop: '1px solid var(--outline-subtle)', margin: '16px 0' }} />
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <img src="/assets/flame_orange.png" width={40} height={40} alt=""
            style={{ display: 'block', flexShrink: 0, borderRadius: 100 }} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ ...T.btnSm, color: 'var(--text-primary)' }}>Get assured ₹10</div>
    ...
    const BillAvatar = ({ children, variant = 'tinted', size = 54 }) => (
      <div style={{
        width: size, height: size, borderRadius: 100,
        // Drives the inline SVG bill glyphs' currentColor so they theme (skill logic).
        color: 'var(--text-secondary)',
        background: variant === 'outline' ? 'var(--surface)' : 'var(--slate-10)',
        border: variant === 'outline' ? '1px solid var(--outline-subtle)' : 'none',
    ...
    /* Themeable bill-ROW avatar (40px) — same logic as the grid tiles: a
       slate-10 chip whose `color` drives the inline SVG glyph's currentColor,
       so it flips dark↔light. Replaces the old baked raster bill_*.png in the
       card-list bill variants (U/V/W), which had dark strokes that vanished on
       dark. Maps each bill's heroImg key → the matching skill glyph. */
    const BILL_ROW_GLYPH = {
      'bill_credit.png': <CreditCardIcon size={20} />,
      'bill_electric.png': <ElectricityIcon size={20} />,
      'bill_mobile.png': <MobileIcon size={20} />,
    };
    const BillRowAvatar = ({ heroImg }) => (
      <BillAvatar size={40}>{BILL_ROW_GLYPH[heroImg] || <MoreIcon size={20} />}</BillAvatar>
    );
    ...
    const BILL_ICONS_SINGLE = ['Credit card', 'Electricity', 'Recharge', 'View more'];
              ...
              {renderedSlides.map((s, i) => (
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    <div>
                      {/* Slide bg is a FIXED light gradient (FY_SCHEMES) that does
                         NOT theme — keep on-card text FIXED dark so it stays legible
                         in dark mode (var(--text-*) would flip white → invisible). */}
              ...
              {renderedSlides.map((s, i) => {
                ...
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    <div>
                      {/* FIXED light mesh bg (fySchemeForSlide) does not theme —
            ...
            <div ref={ref} style={{
              ...
              {renderedSlides.map((s, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {/* FIXED light gradient bg does not theme — keep text FIXED dark. */}
                    <div style={{ ...T.h4, lineHeight: '20px', color: 'rgba(0,0,0,0.9)' }}>{s.title}</div>
                    <div style={{ ...T.caption, color: 'rgba(0,0,0,0.7)', marginTop: 4 }}>{s.sub}</div>
                  </div>
                </div>
    ...
    const FY_G = () => {
      ...
      const squareTile = (s) => (
        <button key={s.title} className="tap" style={{
          ...
          <div>
            {/* FIXED light gradient tile (slideGradient) does not theme —
               keep title/sub FIXED dark so they stay legible in dark mode. */}
            <div style={{ ...T.h4, lineHeight: '20px', color: 'rgba(0,0,0,0.9)' }}>{s.title}</div>
            <div style={{ ...T.caption, color: 'rgba(0,0,0,0.7)', marginTop: 4 }}>{s.sub}</div>
          </div>
                ...
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{
                        ...T.caption, color: 'var(--text-secondary)', marginTop: 2,
                        whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                      }}>{s.sub}</div>
                    </div>
                  </div>
                ...
                return (
                  <div key={i} style={{
                    ...
                    <div style={{
                      ...
                      {/* CTA pill sits on a FIXED-dark hero image — keep it a
                         FIXED white pill with FIXED dark text. var(--surface)
                         goes near-transparent in dark mode → pill vanishes. */}
                      <button className="tap" style={{
                        marginTop: 16,
                  ...
                  <div key={i} style={{
                    ...
                    <div style={{
                      ...
                      <button className="tap" style={{
                        ...
                        ...T.caption, fontWeight: 500, color: '#171A1F', cursor: 'pointer', whiteSpace: 'nowrap',
                      }}>{s.cta}</button>
                    </div>
                  </div>
                );
              ...
              return (
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    {/* CTA pill sits on a FIXED-dark hero image — FIXED white
                       pill. var(--surface) goes near-transparent in dark mode,
                       leaving the fixed-dark label invisible. */}
                    <button className="tap" style={{
    ...
    const MoniesGlyph = ({ size = 14, color = 'currentColor' }) => (
      /* Canonical monies brand mark (Figma node 886:24912), inline so it themes
         via currentColor — black on light, white on dark, matching the skill
         proto's MoniesMark. viewBox 0 0 21 37 (aspect 0.57). */
      <svg width={size * 0.57} height={size} viewBox="0 0 21 37" fill="none"
      ...
      return (
        <PagePad>
          <div style={{ paddingTop: outerPaddingTop }}>
            <div ref={deckRef} className="no-page-swipe" style={{ position: 'relative', height: stackHeight, touchAction: 'none' }}>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: isGlass ? 'rgba(255,255,255,0.28)' : 'var(--surface-solid)',
                    ...
                    {renderContent ? renderContent(it) : (
                      <>
                        ...
                        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
                          ...
                          <div style={{
                            ...T.caption, color: 'var(--text-secondary)',
                            whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                          }}>{it.sub}</div>
                        </div>
                      </>
    ...
    /* titleColor/subColor are FIXED dark (not var(--text-*)): each card's
       surface is a FIXED light mesh over a literal #FFFFFF base that does NOT
       theme, so themed text would flip white in dark mode → invisible. */
    const FY_B_THEMES = [
    ...
    const FY_CardCarousel = ({ cardStyle, overlayGradient, bgType = 'image', slides: slidesProp }) => {
      ...
      return (
        <PagePad>
          {/* 8px above the first card — matches Banking L0's app-bar→card gap (was 16). */}
            ...
            <div style={{
              ...
              <div ref={ref} style={{
                ...
                {renderedSlides.map((s, i) => {
                  ...
                  /* The mesh bg is a FIXED light gradient (FY_SLIDE_SCHEMES over a
                     #FFFFFF base) that does NOT theme — so its text must be FIXED
                     dark, not var(--text-*) (which would flip white → invisible in
                     dark mode). Image/valentino bgs are fixed-dark → fixed white. */
    ...
    const FY_T = () => {
      ...
      return (
        <>
          <div style={{ position: 'relative', marginTop: 'calc(-1 * var(--bar-overlap, 118px))', overflow: 'hidden', background: 'var(--surface)' }}>
            {slides.map((s, p) => (
              <div key={p} aria-hidden style={{
                position: 'absolute', inset: 0, zIndex: 0,
                backgroundImage: `url(/assets/${s.bg})`,
                ...
                return (
                  <div key={i} style={{
                    ...
                    <div style={{
                      ...
                      {/* Visible bg is a FIXED light hero image (fy_hero_white.png)
                         that fades to a FIXED #FFFFFF — it does NOT theme. Keep
                         title/sub FIXED dark so they stay legible in dark mode. */}
    ...
    const AB_A = () => (
      <PagePad>
        <button className="tap" style={{
          ...
          display: 'flex', alignItems: 'center', gap: 12,
          background: 'var(--surface)', border: '1px solid var(--outline-subtle)', borderRadius: 100,
          boxShadow: '0 2px 8px rgba(0,0,0,0.05)',
          cursor: 'pointer', textAlign: 'left',
        }}>
          <img src="/assets/ai_icon.png" width={23} height={23} alt="" style={{ display: 'block', flexShrink: 0 }} />
          <span style={{ ...T.bodySm, color: 'var(--text-tertiary)', lineHeight: '20px', flex: 1, minWidth: 0 }}>
    ...
    const AB_B = () => (
      <PagePad>
        <button className="tap" style={{
          ...
          background: 'var(--surface)', border: '1px solid #D30AD7', borderRadius: 100,
          boxShadow: '0 0 0 4px rgba(211,10,215,0.06)',
          display: 'flex', alignItems: 'center', gap: 12,
          cursor: 'pointer', textAlign: 'left',
        }}>
          <img src="/assets/ai_icon.png" width={23} height={23} alt="" style={{ display: 'block', flexShrink: 0 }} />
          <span style={{ ...T.body, color: 'var(--text-tertiary)', flex: 1 }}>Ask AI banker anything…</span>
        ...
        <div>
          <PagePad>
            <button className="tap" style={{
              ...
              <img src="/assets/ai_icon.png" width={23} height={23} alt="" style={{ display: 'block', flexShrink: 0 }} />
              <span style={{ ...T.bodySm, color: 'var(--text-tertiary)', flex: 1 }}>Ask anything…</span>
            </button>
          </PagePad>
          <Spacer h={16} />
          <div style={{
              ...
              <div style={{
                ...
                {examples.map((e, i) => (
                  <div key={i} style={{
                    ...
                    ...T.bodySm, color: 'var(--text-tertiary)',
                    whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                  }}>{e}</div>
                ))}
              </div>
    ...
    const BL_B = ({ isInCard }) => (
      <PagePad>
        <div style={{ background: 'var(--surface)', boxShadow: CARD_SHADOW, border: CARD_BORDER, borderRadius: 16, padding: 20 }}>
          {isInCard && (
            <>
        ...
        <FY_I
          ...
          renderContent={(it) => (
            <>
              ...
              <div style={{
                ...T.h4, color: 'var(--text-primary)', flexShrink: 0,
                pointerEvents: 'none',
              }}>{it.amount}</div>
            </>
          )} />
    ...
    /* BL_U — Mirrors the canonical "Recharge & bills" composite card
       (BillsCompositeCard): header + ₹0 fee tag, a 4-tile themeable bill grid,
       a divider, then a single "bills due" SUMMARY row (count badge + total),
       replacing the cashback row. No carousel, no pagination. */
    const BL_U = () => (
      <PagePad>
        <div style={{
          background: 'var(--surface)', boxShadow: CARD_SHADOW, border: CARD_BORDER,
          ...
          <div style={{ borderTop: '1px solid var(--outline-subtle)', margin: '16px 0' }} />
          {/* Bills-due summary row — amber count badge + total. */}
          <button className="tap" style={{
            ...
            <div ref={scrollRef} className="scrollbar-hide" style={{
              ...
              {items.map((it, i) => (
                <div key={i} style={{
                  ...
                  <BillRowAvatar heroImg={it.heroImg} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ ...T.btnSm, color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{it.title}</div>
            ...
            <div style={{ margin: '4px 24px 0', position: 'relative', height: 1.5 }}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  background: 'radial-gradient(ellipse 100% 100% at center, var(--outline-bold) 0%, transparent 75%)',
                  borderRadius: 1,
                }} />
              </div>
            </div>
    ...
    const BillsSection = ({ variant, isInCard }) => {
      const C = { A: BL_A, B: BL_B, C: BL_C, D: BL_D, E: BL_E, F: BL_F, J: BL_J, L: BL_L, M: BL_M, P: BL_P, R: BL_R, S: BL_S, T: BL_T, U: BL_U, W: BL_W }[variant];
    ...
    const SparkHeroCard = ({ titles = SPARK_TITLES }) => {
      ...
      return (
        <button ref={rootRef} className="tap" style={{
          ...
          background: 'var(--surface)', border: CARD_BORDER, boxShadow: CARD_SHADOW,
          textAlign: 'left', cursor: 'pointer',
          display: 'flex', alignItems: 'center', gap: 16,
          position: 'relative', overflow: 'hidden',
        }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ ...T.caption, color: 'var(--text-tertiary)' }}>Spark</div>
      ...
      <>
        <PagePad>
          <button className="tap" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ ...T.h4, color: 'var(--text-primary)' }}>Play &amp; win</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 }}>
                  <img src="/assets/rank_badge_1.png" width={17} height={17} alt="" style={{ display: 'block' }} />
                  <span style={{ ...T.caption, color: 'var(--text-secondary)' }}>Aman leading with</span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2 }}>
                    <span style={{ ...T.caption, color: 'var(--text-primary)', fontWeight: 500 }}><MoniesGlyph size={12} /> 4,657</span>
                  </span>
                </div>
              </div>
              <Chevron />
        ...
        <PagePad>
          <button className="tap" style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ ...T.caption, color: 'var(--text-secondary)', marginTop: 2 }}>
                Win up to <span style={{ color: 'var(--text-primary)', fontWeight: 500 }}><MoniesGlyph size={12} /> 4,657</span>
              </div>
            </div>
            <div style={{
      ...
      <>
        <PagePad>
          <button className="tap" style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ ...T.h4 }}>Fire games</div>
              <div style={{ ...T.caption, color: 'var(--text-secondary)', marginTop: 4 }}>
                Win up to <span style={{ color: 'var(--text-primary)', fontWeight: 500 }}><MoniesGlyph size={12} /> 4,657</span>
              </div>
              <div style={{ marginTop: 12 }}>
                <span style={{
        ...
        <div style={{
          ...
          <button className="tap" style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1, minWidth: 0 }}>
              <div style={{ ...T.caption, color: 'var(--text-tertiary)' }}>Fires</div>
              <div style={{ ...T.btnSm, color: 'var(--text-primary)' }}>6 live</div>
            </div>
          </button>
          ...
          <button className="tap" style={{
            ...
            <img src="/assets/monies_icon.png" width={36} height={36} alt=""
              style={{ display: 'block', flexShrink: 0 }} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1, minWidth: 0 }}>
              <div style={{ ...T.caption, color: 'var(--text-tertiary)' }}>Monies</div>
              <div style={{
                ...T.btnSm, color: 'var(--text-primary)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {RW_O_ROWS.map((row) => (
            <button key={row.label} className="tap" style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ ...T.caption, color: 'var(--text-tertiary)' }}>{row.label}</div>
                <div style={{
                  ...T.h3, color: 'var(--text-primary)', marginTop: 2,
      ...
      const moniesLandscape = (
        <button className="tap" style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ ...T.h3, color: 'var(--text-primary)', marginTop: 4,
              display: 'inline-flex', alignItems: 'center', gap: 4 }}>
      ...
      <PagePad>
        <button className="tap" style={{
          ...
          <img src="/assets/monies_icon.png" width={32} height={32} alt=""
            style={{ display: 'block', flexShrink: 0 }} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ ...T.caption, color: 'var(--text-tertiary)' }}>Monies balance</div>
            <div style={{
              ...T.h4, color: 'var(--text-primary)', marginTop: 2,
              display: 'inline-flex', alignItems: 'center', gap: 4,
            }}>
      ...
      <PagePad>
        <button className="tap" style={{
          ...
          <img src="/assets/monies_icon.png" width={32} height={32} alt=""
            style={{ display: 'block', flexShrink: 0 }} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ ...T.h4, color: 'var(--text-primary)' }}>Monies</div>
            <div style={{ ...T.caption, color: 'var(--text-tertiary)', marginTop: 2 }}>rewarded at 1%</div>
          </div>
          <div style={{
            ...T.h4, color: 'var(--text-primary)',
            display: 'inline-flex', alignItems: 'center', gap: 4,
          }}>
            <MoniesGlyph size={14} /> 66,522
          </div>
    ...
    /* MN_E — Deposit card: Figma artwork bg (magenta gradient + coin illustration),
       matches Figma node 9988:8144. Text stacked left, 20px padding, white on image. */
    const MN_E = () => (
      <PagePad>
        <button className="tap" style={{
          width: '100%', height: 104, padding: '20px 16px',
          borderRadius: 16, border: 'none',
          background: `url(/assets/monies_deposit_bg.png) center / cover no-repeat`,
          boxShadow: CARD_SHADOW,
          textAlign: 'left', cursor: 'pointer',
          display: 'flex', flexDirection: 'column', justifyContent: 'center',
          gap: 4, position: 'relative', overflow: 'hidden',
        }}>
          <div style={{ ...T.caption, color: 'rgba(255,255,255,0.85)' }}>monies balance</div>
          <div style={{
            ...T.h3, color: '#FFFFFF',
            display: 'inline-flex', alignItems: 'center', gap: 4,
          }}>
            <MoniesGlyph size={16} color="#FFFFFF" /> 66,522
          </div>
          <div style={{ ...T.caption, color: 'rgba(255,255,255,0.85)' }}>
            you're at 1% reward rate
          </div>
        </button>
      </PagePad>
    );

    /* MN_F — Deposit card + milestone nudge (overlaid on same artwork) */
    const MN_F = () => (
      <PagePad>
        <button className="tap" style={{
          width: '100%', height: 104, padding: '20px 16px',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{
              ...T.caption, color: 'rgba(255,255,255,0.85)', fontSize: 11,
            }}>
              ₹20k away from 1.5%
            </div>
            <div style={{
              flex: 1, maxWidth: 80, height: 3, borderRadius: 4,
              background: 'rgba(255,255,255,0.25)',
            }}>
              <div style={{
                height: '100%', width: '67%', borderRadius: 4,
    ...
    /* MN_G — CSS gradient approximation of the deposit card (no image dependency).
       Hot pink → magenta diagonal with radial glow highlights. */
    const MN_G = () => (
      <PagePad>
        <button className="tap" style={{
          width: '100%', height: 104, padding: '20px 16px',
          borderRadius: 16, border: 'none',
          background: `
            radial-gradient(ellipse 60% 80% at 85% 40%, rgba(255,255,255,0.15) 0%, transparent 60%),
            radial-gradient(ellipse 50% 70% at 75% 90%, rgba(255,180,255,0.2) 0%, transparent 50%),
            linear-gradient(135deg, #E8106E 0%, #D30AD7 50%, #B80CCF 100%)
          `,
          boxShadow: CARD_SHADOW,
          textAlign: 'left', cursor: 'pointer',
          display: 'flex', flexDirection: 'column', justifyContent: 'center',
    ...
    /* MN_H — Brand-to-blue gradient (V-500 → Blue-500 diagonal) with glow.
       Cooler, more premium feel. */
    const MN_H = () => (
      <PagePad>
        <button className="tap" style={{
          width: '100%', height: 104, padding: '20px 16px',
          borderRadius: 16, border: 'none',
          background: `
            radial-gradient(ellipse 50% 70% at 80% 30%, rgba(255,255,255,0.12) 0%, transparent 60%),
            linear-gradient(135deg, #D30AD7 0%, #7B3BDB 45%, #2B6ACF 100%)
          `,
          boxShadow: CARD_SHADOW,
          textAlign: 'left', cursor: 'pointer',
          display: 'flex', flexDirection: 'column', justifyContent: 'center',
    ...
    /* MoniesInline — dispatcher for monies sub-variants inside RW_Y */
    const MONIES_VARIANTS = { A: MN_A, B: MN_B, C: MN_C, D: MN_D, E: MN_E, F: MN_F, G: MN_G, H: MN_H };
    const MoniesInline = ({ variant = 'A' }) => {
      const C = MONIES_VARIANTS[variant] || MN_A;
      return <C />;
    };
    ...
    const RW_Q = () => {
      ...
      const flatTile = (subtext, title, iconSrc, iconSize = 44) => (
        <button className="tap" style={{
          ...
          background: 'var(--surface)', border: '1px solid var(--outline-bold)', boxShadow: 'none',
          textAlign: 'left', cursor: 'pointer',
    ...
    const RW_Y_Card = ({ type, sharedCycle, visible }) => {
      ...
      const caps = isFire ? ['Play & win', 'Aman leading'] : ['Sparks', 'Sparks'];
      const heads = isFire
        ? [{ text: '3 fires' }, { node: <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><MoniesGlyph size={16} color="#FFFFFF" /> 8,435</span> }]
        : [{ text: 'Save ₹1,600' }, { text: '5 drops live' }];
      const capSlots = [caps[0], caps[1], caps[0], caps[1]];
      const headSlots = [heads[0], heads[1], heads[0], heads[1]];
      const tickIdx = cycle % 4;
      const isSnap = tickIdx === 0 && cycle >= 4;
      const CAP_H = 16, HEAD_H = 24;
      const maskStyle = {
        maskImage: 'linear-gradient(to bottom, transparent 0%, black 18%, black 82%, transparent 100%)',
        WebkitMaskImage: 'linear-gradient(to bottom, transparent 0%, black 18%, black 82%, transparent 100%)',
      };
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              {headSlots.map((h, i) => (
                <div key={i} style={{ height: HEAD_H, ...T.h3, color: '#FFFFFF' }}>{h.node || h.text}</div>
              ))}
            </div>
          </div>
        </div>
    ...
    const RewardsSection = ({ variant, isInCard, headerStyle, moniesVariant }) => {
      ...
      return <C isInCard={isInCard} headerStyle={headerStyle} moniesVariant={moniesVariant} />;
    };
      ...
      <PagePad>
        <div style={{ background: 'var(--surface)', boxShadow: CARD_SHADOW, border: CARD_BORDER, borderRadius: 16, padding: 24 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            <div>
              <div style={{ ...T.btnSm, color: 'var(--text-tertiary)' }}>May spends</div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{
                  ...T.h3, color: 'var(--text-primary)', marginTop: 4,
                }}>₹18,400</div>
                <div style={{
    ...
    const ST_G = () => {
      ...
      return (
        <PagePad>
          <div style={{ background: 'var(--surface)', boxShadow: CARD_SHADOW, border: CARD_BORDER, borderRadius: 16, padding: 20, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
              <div>
                <div style={{ ...T.btnSm, color: 'var(--text-tertiary)' }}>May spends</div>
                <div style={{ fontFamily: 'Rubik', fontSize: 28, fontWeight: 500, lineHeight: '32px', letterSpacing: '-0.2px', color: 'var(--text-primary)', marginTop: 4 }}>₹18,400</div>
              </div>
              <span style={{
                ...T.caption, fontWeight: 500,
    ...
    const ST_I = () => {
      ...
      return (
        <PagePad>
          <div style={{ background: 'var(--surface)', boxShadow: CARD_SHADOW, border: CARD_BORDER, borderRadius: 16, padding: 16, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
              <div>
                <div style={T.caption}>May spends</div>
              ...
              {SPEND_CATEGORIES.map(c => (
                <div key={c.label} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <img src={`/assets/${c.icon}`} width={40} height={40} alt=""
                    style={{ display: 'block', flexShrink: 0 }} />
                  <div style={{ flex: 1, minWidth: 0, ...T.bodySm, fontWeight: 500, color: 'var(--text-primary)' }}>{c.label}</div>
        ...
        <PagePad>
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0, ...T.caption, color: 'var(--text-secondary)' }}>
                {SPEND_INSIGHT.text}
              </div>
            </div>
          </div>
    ...
    const MR_A = ({ showInsurance = true }) => (
      <PagePad>
        ...
        {showInsurance && (
          /* Horizontal insurance card — illustration to the RIGHT of the text;
             sits BELOW the autopay / credit-score grid. */
          <button className="tap relative overflow-hidden" style={{
            background: 'var(--surface)', boxShadow: CARD_SHADOW, border: CARD_BORDER,
            borderRadius: 16, width: '100%', padding: '16px 16px 16px 20px', marginTop: 16,
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        ...
        <div style={{ paddingLeft: 12, paddingRight: 12 }}>
          {MR_LIST_ITEMS.map((row, i) => (
            <MoreRow key={row.label} isFirst={i === 0}>
              <span style={{ ...T.body, color: 'var(--text-primary)', flex: 1, minWidth: 0 }}>{row.label}</span>
              <span style={{ ...T.btnSm, color: 'var(--text-tertiary)' }}>{row.value}</span>
            </MoreRow>
          ))}
        </div>
      </PagePad>
    ...
    const MoreSection = ({ variant, isInCard, showInsurance }) => {
      const C = { A: MR_A, B: MR_B, D: MR_D }[variant] || MR_A;
      return <C isInCard={isInCard} showInsurance={showInsurance} />;
    };
    ...
    const ExplorePage = ({ sections, headerStyle, activeSection, separateMore, autoScroll, onScrollPast, showInsurance }) => {
      ...
      return (
        <ScreenShell transparentAppBar={isGradientFY} darkBg={sections.forYou === 'L' || sections.forYou === 'F' || sections.forYou === 'N'}
          ...
          <KioskWrap>
          {/* 24px of air above AI banker in ALL scenarios (explore-base, per user
             — replaces the old conditional 20 / 16 / 0 pre-banker spacers). */}
          ...
          <SectionBlock id="rewards" active={isActive('rewards')}>
            ...
            <SectionWrap title="Rewards & benefits"
              ...
              <RewardsSection variant={sections.rewards} isInCard={isInCard} headerStyle={headerStyle} moniesVariant={sections.monies} />
            </SectionWrap>
          </SectionBlock>
              ...
              {!separateMore && (
                <>
                  ...
                  <SectionBlock id="more" active={isActive('more')}>
                    <MoreSection variant={sections.more} isInCard={isInCard} showInsurance={showInsurance} />
                  </SectionBlock>
                </>
              )}
            </SectionWrap>
    ...
    const FT_B = () => (
      <PagePad>
        <button className="tap" style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ ...T.caption, color: 'var(--text-tertiary)' }}>Invite &amp; earn</div>
            <div style={{ ...T.h3, color: 'var(--text-primary)', marginTop: 2 }}>
              ₹500 per friend
            </div>
          </div>
      ...
      <div style={{
        ...
        <button
          ...
          <span style={{
            ...
            width: 16, height: 16, borderRadius: 100, background: 'var(--surface)',
    ...
    /* Page-level option as an apply/disable PILL (replaces the switch rows).
       Active = filled Valentino tint; inactive = quiet outline. */
    const LayoutPill = ({ label, active, onClick }) => (
      <button onClick={onClick} className={'layout-pill' + (active ? ' active' : '')}>
        {label}
      </button>
    );
    ...
    const DebugPanel = ({
      ...
      showInsurance, onShowInsuranceToggle,
      theme, onThemeToggle,
    }) => {
      /* Section-variants group is collapsible, OPEN by default — all variant
         changes happen from inside it. */
      ...
      return (
        <div className="debug-panel">
          ...
          <div className="layout-pills">
            ...
            <LayoutPill label="Archived variants" active={showArchived} onClick={onShowArchivedToggle} />
            <LayoutPill label="Highlight spacing" active={highlight} onClick={onHighlightToggle} />
            <LayoutPill label="Dark mode" active={theme === 'dark'} onClick={onThemeToggle} />
          </div>
            ...
            <div style={{ marginTop: 16 }}>
              {Object.keys(SPACING_OPTIONS).map(key => (
                <div className="var-row" key={key} style={{ alignItems: 'center' }}>
                  <div style={{ ...T.bodySm, color: 'var(--text-primary)', flex: 1, minWidth: 0, paddingRight: 8 }}>
                    {SPACING_LABELS[key]}
                  </div>
    ...
    function ExploreL0({ onScrollChange, onDarkTopChange }) {
      ...
      // Desktop debug dock is PORTAL'd to the skill's [data-theme] stage (not
      // rendered inline): the phone chassis carries a CSS transform, which would
      // otherwise trap this position:fixed panel inside the clipped phone instead
      // of docking it to the viewport edge. Portaling to the stage also lets the
      // dock inherit dark/light theming. Resolved after mount (DOM committed).
      const [stageEl, setStageEl] = useState(null);
      React.useEffect(() => { setStageEl(document.querySelector('[data-theme]')); }, []);
      // App theme (from the skill's ThemeContext, provided around the pod tree) —
      // drives the in-panel dark-mode toggle on mobile, where the skill's own
      // bottom-left toggle is hidden.
      const { theme, toggleTheme } = useTheme();
      const [separateMore, setSeparateMore] = useState(true);
      const [showInsurance, setShowInsurance] = useState(true); // debug toggle: remove the More-section insurance card
      ...
      /* Open the page-composition editor by tapping the EXPLORE item in the
         (inherited, shared) bottom nav while Explore is the active page. The
         skill BottomNav renders each item as `.slice-bnav-slot[data-pod=...]`
         with aria-current="page" on the active one — so we listen at the
         document level (NO edit to the shared nav, it stays linked) and open
         when the explore slot is tapped while already current. The legacy
         'open-debug-drawer' window event is kept as a secondary hook. */
      React.useEffect(() => {
        const onOpen = () => setDrawerOpen(true);
        window.addEventListener('open-debug-drawer', onOpen);
        const onDocClick = (e) => {
          const slot = e.target.closest && e.target.closest('.slice-bnav-slot[data-pod="explore"]');
          if (slot && slot.getAttribute('aria-current') === 'page') setDrawerOpen((v) => !v);
        };
        // CAPTURE phase: read aria-current BEFORE the skill nav handles the click,
        // so the sheet opens ONLY when Explore was ALREADY current (2nd tap). A
        // first tap from another pod just navigates (aria-current not yet 'page').
        document.addEventListener('click', onDocClick, true);
        return () => {
          window.removeEventListener('open-debug-drawer', onOpen);
...
/* Reset the extra-scroll flag any time the drawer closes — by any path
      ...
      const debugPanel = (
        <DebugPanel
          ...
          onShowInsuranceToggle={() => setShowInsurance(s => !s)}
          theme={theme}
          ...
          >
            {useOriginal
              ...
              : <ExplorePage
                  ...
                  separateMore={separateMore}
                  autoScroll={autoScroll}
                  onScrollPast={setHeroScrolledPast}
                  showInsurance={showInsurance}
                />}
            ...
            {stageEl && createPortal(
              isMobile ? (
                /* MOBILE: dark debug bottom sheet, FIXED to the viewport bottom and
                   portaled to the skill stage so it sits ABOVE the bottom nav (the
                   nav lives inside the transformed pager, which would otherwise
                   stack over an in-pod sheet). Dark on purpose; no backdrop; only
                   the ✕ or a downward drag closes. */
                <div
                  /* INVERT in dark mode: when the app is dark the sheet flips to a
                     LIGHT surface (white bg + data-theme=light → light tokens via
                     the project [data-theme=light] block) so it contrasts against
                     the dark app. In light mode it stays the dark debug sheet. */
                  className={'ep-editor-sheet' + (theme === 'dark' ? ' light' : '')}
                  data-theme={theme === 'dark' ? 'light' : undefined}
                  style={{
                    position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 9000,
                    height: '58%',
                    background: theme === 'dark' ? '#FFFFFF' : '#171A1F',
                    borderRadius: '20px 20px 0 0',
                    boxShadow: drawerOpen ? '0 -12px 40px rgba(0,0,0,0.35)' : 'none',
                    transform: drawerOpen ? 'translateY(0)' : 'translateY(100%)',
                    transition: 'transform 320ms cubic-bezier(0.32,0.72,0,1), box-shadow 280ms ease',
                    display: 'flex', flexDirection: 'column', overflow: 'hidden',
                    pointerEvents: drawerOpen ? 'auto' : 'none',
                  }}
                >
                  <DrawerDragHandle onClose={() => setDrawerOpen(false)} />
                  <div className="ep-editor-scroll scrollbar-hide" style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
                    {debugPanel}
                  </div>
                  <button
                    ...
                    style={{
                      ...
                      background: theme === 'dark' ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.14)',
                      color: theme === 'dark' ? 'rgba(0,0,0,0.7)' : '#fff', cursor: 'pointer', zIndex: 5,
                ...
                /* DESKTOP: full-height page-composition panel docked on the RIGHT
                   HALF of the page — a FLEX ITEM in the skill stage (NOT a floating
                   box), so the phone shifts left and this takes the right, like the
                   standalone split. data-theme="light" LOCKS it light — the app's
                   dark/light toggle must NOT affect the debug panel on desktop. */
                /* DESKTOP dock: data-theme="light" LOCKS it light — the app's
                   dark/light toggle must NOT change the desktop panel. The
                   dark-mode control now lives as a pill inside the panel. */
                <div className="ep-desktop-dock" data-theme="light">
                  {debugPanel}
                </div>
              ),
              stageEl
